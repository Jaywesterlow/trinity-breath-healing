/**
 * `use:magnetic` — magnetic cursor-follow for a carousel's centre card.
 * Translate only — no rotation, no perspective, no tilt.
 *
 ...
 * CarouselCard.svelte) can compose the magnet offset with its independent
 * hover-scale transform on the same `transform` declaration without either
 * one clobbering the other.
 *
 * Contract:
 *   - Only attaches when `enabled` is true (only ever pass this for the
 *     centre card) AND the device actually has a fine pointer with real
 ...
 *     real bug.
 *   - rAF-throttles pointermove, same reasoning as CarouselEngine's own
 *     drag handler: pointer events can outrun the display.
 */
...
export function magnetic(node: HTMLElement, options: MagneticOptions) {
	...
	function releaseTo(x: string, y: string): void {
		...
		node.style.removeProperty('--carousel-card-transition-duration');
	...
	function onPointerMove(e: PointerEvent): void {
		...
		rafId = requestAnimationFrame(() => {
			...
			// over.
			node.style.setProperty('--carousel-card-transition-duration', `${MAGNET_TRACK_MS}ms`);
