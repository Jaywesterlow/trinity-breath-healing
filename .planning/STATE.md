...
| **Plain CSS** — single `static/global.css` + Svelte scoped styles (pivot from Tailwind v4, commit `a7f96e1`) — **SUPERSEDED 2026-07-12** (D-09, see CLAUDE.md): reversed back to **Tailwind v4 + shadcn-svelte** in Slice 1 to get shadcn's component model instead of hand-rolling equivalents; `static/global.css` deleted, tokens moved to `src/app.css` `@theme` | Zero build-tool churn, Svelte scoped CSS sufficient, design-token system via CSS variables | PROJECT.md + commit a7f96e1 |
...
| 260808-ctj | Desktop click-to-jump on the Behandelingen carousel. Transparent overlay button on the two visible side cards (`positions[i] === ±1`) reuses the existing `goTo`/`commitSteps` cascade — no new position maths. Desktop-only; mobile keeps swipe as its only card-level gesture. `dragMoved` slop guard stops a mouse drag also firing a jump on release; `cascading` guard added to `goTo`. Keyboard path unchanged and already complete via the dots and Prev/Next. 6 new Playwright tests. | 2026-08-08 | pending | [260808-ctj-desktop-click-to-jump-behandelingen](./quick/260808-ctj-desktop-click-to-jump-behandelingen/) |
| 260809-jwg | Finished the interrupted 2026-08-09 HANDOFF.md update. The previous session was cut off after writing the header note and the Branches rewrite but before the section that header points at, so the file forwarded readers to a non-existent "Carousel session 2026-08-08/09" while the 2026-08-07 section still documented `commitSteps`/`FAST_STEP_MS`/`MAX_FLING_STEPS` — all deleted by `b4d69b7`/`8e0e582`. Wrote the missing section (coast+critically-damped-latch physics, the constants table mapped to owner feedback, the drag-band rotated-bbox trap, the desktop geometry churn), retitled and corrected the older section rather than deleting it, and committed the untracked `RESEARCH-carousel-physics-gsap.md`. TESTKAART diagnostic left live on purpose — documented as blocking PR #10, reverting it is the owner's call. | 2026-08-09 | pending | [260809-jwg-finish-handoff-carousel-session](./quick/260809-jwg-finish-handoff-carousel-session/) |
