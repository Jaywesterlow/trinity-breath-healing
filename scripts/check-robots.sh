...
#        Defaults to the prerendered output, because robots.txt is generated by
#        src/routes/robots.txt/+server.ts and there is no static file to check.
#        Run after `npm run build`.
#
# Wired into the CI build-and-audit job by Plan 08.
set -euo pipefail

ROBOTS_FILE="${1:-.svelte-kit/output/prerendered/pages/robots.txt}"

if [ ! -f "$ROBOTS_FILE" ]; then
  echo "ERROR: robots.txt not found at $ROBOTS_FILE" >&2
  echo "       robots.txt is generated — run 'npm run build' first, or pass a path." >&2
  exit 1
fi
